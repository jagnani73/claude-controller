...
- **`CLAUDE_CODE_MINIMUM_VERSION` (2.1.181) is below the arrow-then-Enter race fixes** (2.1.235/2.1.247) that our 35ms `CHUNK_DELAY_MS` now rides on. A CLI in 2.1.181–2.1.234 clears the floor while reintroducing the races. Unverified — don't raise the floor without demonstrating the race actually breaks a relay.
